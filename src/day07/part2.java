...
public class part2 {
    ...
    public boolean calculate(ArrayList<Long> values, long target, int mults) {
        ...
        if (!(mults == values.size() - 1)) {
            ...
            ArrayList<Long> values_concat = new ArrayList<>(values_old);
            values_concat.add(0, Long.parseLong(a + "" + b));
            answer = answer || calculate(values_concat, target, mults);
        } if (!(mults == 0)) {
