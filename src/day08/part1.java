        ...
        for (Map.Entry<Character, ArrayList<Coord>> entry : antennas.entrySet()) {
            ...
            for (Coord a : value) {
                for (Coord b : value) {
                    ...
                    int diff_x = a.x - b.x;
                    int diff_y = a.y - b.y;
                    Coord new_a = new Coord(a.x + diff_x, a.y + diff_y);
                    if (!coord_contain(antinodes,new_a) && inbounds(a.x + diff_x, a.y + diff_y, dimension)) {
                        antinodes.add(new_a);
                    }
                    Coord new_b = new Coord(b.x - diff_x, b.y - diff_y);
                    if (!coord_contain(antinodes,new_b) && inbounds(b.x - diff_x, b.y - diff_y, dimension)) {
                        antinodes.add(new_b);
                    }
                }
            }
